...
def CellChamberModel(x,y):
    ...
    Zcpe = (1/(FitRef[1]))*((1j*omega)**(-FitRef[2]))
    Cm=(1/(1j*omega*x[3]))
    Zm = np.divide((x[4]*Cm),(x[4]+Cm))
    value1 =  np.add(Zcpe,Zm)
    value= np.add(value1,FitRef[0])
...
class App(QMainWindow):
    ...
    def on_click_ImpedanceAnalyserData(self):
        ...
        ####    Cell Chamber Electrode Fitting
                ...
                RbCellChamber=FitRef[0]*(CELLCONSTANT_REFERENCE/CELLCONSTANT_CELLCHAMBER_BASO)
